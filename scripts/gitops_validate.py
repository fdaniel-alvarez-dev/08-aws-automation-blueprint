...
def main() -> int:
    if not GITOPS_DIR.exists():
        print("Missing gitops/ directory.")
        return 2

    yaml_files = sorted(p for p in GITOPS_DIR.rglob("*.yaml") if p.is_file())
    if not yaml_files:
        print("No YAML files found under gitops/.")
        return 2
